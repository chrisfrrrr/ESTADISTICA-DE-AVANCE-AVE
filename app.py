...
load_dotenv()
st.set_page_config(page_title='AVE Monitor Académico Pro 4.0 FIX7 FIX6', page_icon='assets/app_icon.ico', layout='wide')
...
with st.sidebar:
    ...
    st.caption('AVE Monitor Académico Pro 4.0 FIX7 FIX6')
    ...
    if st.button('Probar conexión / cargar cursos', use_container_width=True, type='primary'):
        try:
            ...
            st.caption(f'URL normalizada usada: {c.base_url}')
            me = c.whoami()
            courses = c.courses()
...
st.markdown('<p class="ave-title">AVE Monitor Académico Pro 4.0 FIX7 FIX6</p>', unsafe_allow_html=True)
